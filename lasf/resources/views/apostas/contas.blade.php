            ...
            <table class="table table-sm table-striped table-bordered">
                ...
                <tbody>
                    @foreach($contas as $conta)
                        <tr>
                            <form action="{{ route('apostasContas.update', $conta->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                @foreach($colunas as $coluna)
                                    <td>
                                        @if($coluna == 'id' || $coluna == 'data')
                                            @if($coluna == 'data')
                                                {{ \Carbon\Carbon::parse($conta->$coluna)->format('d/m/Y') }}
                                            @else
                                                {{ $conta->$coluna }}
                                            @endif
                                        @else
                            ...
                            <td>
                                <form action="{{ route('apostasContas.destroy', $conta->id) }}" method="POST" >
                                    @csrf
                                    @method('DELETE')
                    ...
                    @endforeach
                    <tr>
                        <form action="{{ route('apostasContas.store') }}" method="POST">
                            @csrf
                            @foreach ($colunas as $coluna)
                                @if($coluna == 'id' || $coluna == 'data')
                                    <td> </td>
