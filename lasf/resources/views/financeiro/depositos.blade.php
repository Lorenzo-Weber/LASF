            ...
            <table class="table table-sm table-striped table-bordered">
                ...
                <tbody>
                    @foreach($depositos as $deposito)
                        <tr>
                            <form action="{{ route('financeiroDepositos.update', $deposito->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                @foreach($colunas as $coluna)
                                    <td>
                            ...
                            <td>
                                <form action="{{ route('financeiroDepositos.destroy', $deposito->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @endforeach
                    <tr>
                        <form action="{{ route('financeiroDepositos.store') }}" method="POST">
                            @csrf
                            @foreach ($colunas as $coluna)
                                @if($coluna == 'id' || $coluna == 'data')
                                    <td> </td>
